...
interface ProductoDao {
    ...
    @Query("SELECT * from Producto WHERE cod_negocio = :cod_negocio  AND sku!='' AND estado='1'  ORDER BY descripcion")
    ...
    @Query("SELECT * from Producto WHERE cod_negocio = :cod_negocio AND cod_categoria = :cod_categoria  AND sku!=''  ORDER BY upper(descripcion) ASC")
    ...
    @Query("SELECT count(1) from Producto WHERE estadoGuardado = 1 and estado=1")
